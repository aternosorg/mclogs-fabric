...
public class MclogsListCommand implements Command {
    ...
    private int execute(Source source, String command) {
        try {
            ...
            if (total == 0) {
                message = Text.literal("No logs or crash reports found.").formatted(Formatting.RED);
            }
            ...
            MutableText error = Text.literal("An error occurred. Check your log for more details.");
    ...
    private MutableText title(String title) {
        return Text.literal(title).setStyle(Style.EMPTY.withFormatting(Formatting.UNDERLINE));
    }
